        ...
        <div class="row align-items-center">
            <div class="col-lg-8">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-2">
                        ...
                        <li class="breadcrumb-item active" aria-current="page" style="color: var(--warning-800, #92400e);">Edit</li>
                    </ol>
                </nav>
            ...
            <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                @if($event->status === 'published')
                    <span class="badge bg-success px-3 py-2"><i class="fas fa-check-circle me-1"></i>Published</span>
                @elseif($event->status === 'pending')
                    <span class="badge bg-warning text-dark px-3 py-2"><i class="fas fa-clock me-1"></i>Pending</span>
                @else
                    <span class="badge bg-secondary px-3 py-2"><i class="fas fa-file-alt me-1"></i>Draft</span>
                @endif
            </div>
        </div>
    </div>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('organizer.events.update', $event) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control form-control-modern @error('short_description') is-invalid @enderror" 
                                              ...
                                              rows="2">{{ old('short_description', $event->short_description) }}</textarea>
                            ...
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control form-control-modern @error('address') is-invalid @enderror" 
                                              id="address" 
                                              name="address" 
                                              rows="2">{{ old('address', $event->address) }}</textarea>
                                    @error('address')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row g-4">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select form-select-modern @error('event_type') is-invalid @enderror" 
                                        ...
                                        <option value="free" {{ old('event_type', $event->event_type) == 'free' ? 'selected' : '' }}>Gratis</option>
                                        <option value="paid" {{ old('event_type', $event->event_type) == 'paid' ? 'selected' : '' }}>Berbayar</option>
                                    </select>
                                    @error('event_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-4" id="price-field" style="display: {{ old('event_type', $event->event_type) == 'paid' ? 'block' : 'none' }};">
                                ...
                                <div class="col-12">
                                    <label class="form-label-modern">Gambar Event</label>
                                    @if($event->image)
                                        <div class="current-image">
                                            <img src="{{ Storage::url($event->image) }}" alt="Current image">
                                            <div class="form-hint mt-2">Gambar saat ini</div>
                                        </div>
                                    @endif
                                    <label for="image" class="file-upload-area d-block">
                                        ...
                                        <div class="fw-semibold">Klik untuk upload gambar baru</div>
                            ...
                            <div class="row g-4">
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control form-control-modern @error('requirements') is-invalid @enderror" 
                                              id="requirements" 
                                              name="requirements" 
                                              rows="3">{{ old('requirements', is_array($event->requirements) ? implode("\n", $event->requirements) : $event->requirements) }}</textarea>
                                    <div class="form-hint">Satu persyaratan per baris</div>
                                    @error('requirements')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control form-control-modern @error('contact_info') is-invalid @enderror" 
                                              id="contact_info" 
                                              name="contact_info" 
                                              rows="2">{{ old('contact_info', $event->contact_info) }}</textarea>
                                    @error('contact_info')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="d-flex justify-content-between pt-4 border-top">
                            <a href="{{ route('organizer.events.show', $event) }}" class="btn btn-secondary-modern">
